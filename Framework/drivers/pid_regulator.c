...
extern fw_PID_Regulator_t rotateYawSpeedPID;
...
{
	return PID_PROCESS_Double(&yawPositionPID,&rotateYawSpeedPID,target,position_feedback,velocity_feedback);
}
int16_t ProcessPitchPID(float target, float position_feedback, float velocity_feedback)
{
	...
	//return PID_PITCH_PROCESS_Double(pitchPositionPID,pitchSpeedPID,target,position_feedback,velocity_feedback);
}
...
int16_t PID_PITCH_PROCESS_Double(fw_PID_Regulator_t pid_position,fw_PID_Regulator_t pid_speed,float target, float position_feedback, float velocity_feedback)
{
	//position		
	pid_position.target = target;
	pid_position.feedback = position_feedback;
